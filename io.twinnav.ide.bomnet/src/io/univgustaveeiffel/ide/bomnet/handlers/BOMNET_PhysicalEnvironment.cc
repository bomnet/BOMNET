...
//CODE ADDED BY BOMNET
        else if (!strcmp(shapeType, "ifcPolyhedron")) {
                    std::vector<Coord> points;
                    while (shapeTokenizer.hasMoreTokens()) {
                        Coord point;
                        point.x = atof(shapeTokenizer.nextToken());
                        if ((tok = shapeTokenizer.nextToken()) == nullptr)
                            throw cRuntimeError("Missing IfcPolyhedron y at %s", element->getSourceLocation());
                        point.y = atof(tok);
                        if ((tok = shapeTokenizer.nextToken()) == nullptr)
                            throw cRuntimeError("Missing IfcPolyhedron z at %s", element->getSourceLocation());
                        point.z = atof(tok);
                        points.push_back(point);
                    }
                    if (points.size() < 4)
                        throw cRuntimeError("IfcPolyhedron needs at least four points at %s", element->getSourceLocation());
                    size = Box::computeBoundingBox(points).getSize();
                    convertPoints(points);
                    // ADDED
                    std::vector<CustomFace> customFaces;
                    const char *facesAttribute = element-> getAttribute("faces");

                    cStringTokenizer f_tokenizer(facesAttribute);

                    while (f_tokenizer.hasMoreTokens()) {
                                    CustomFace custom_face;
                                    custom_face.p1 = atoi(f_tokenizer.nextToken());
                                    custom_face.p2 = atoi(f_tokenizer.nextToken());
                                    custom_face.p3 = atoi(f_tokenizer.nextToken());
                                    customFaces.push_back(custom_face);
                                            }

                                convertPoints(points);
                                shape = new IfcPolyhedron(points, customFaces);
                                shapes.push_back(shape);
                }
//END OF CODE ADDED BY BOMNET
        else {
